    ...
    <div className="w-full">
      ...
      <div className="space-y-6">
        {bullets.map((item, idx) => (
          <div key={idx} className="bg-bg-card border border-border-primary rounded-2xl overflow-hidden shadow-sm backdrop-blur-sm group hover:border-brand-primary/20 transition-all duration-300">
            {/* Header Strip */}
            <div className="bg-bg-dark/50 px-6 py-4 border-b border-border-secondary flex justify-between items-center">
              <span className="text-xs font-bold text-text-secondary uppercase tracking-widest group-hover:text-brand-primary transition-colors">
                Suggestion #{idx + 1}
              </span>
              <div className="flex gap-2">
                {item.feedback_tag && (
                  <span className="px-3 py-1 bg-text-primary/5 text-text-secondary text-[10px] font-bold uppercase rounded-lg border border-border-primary backdrop-blur-md">
                    {item.feedback_tag}
                  </span>
                )}
              </div>
            ...
            <div className="p-8">
              ...
              <div className="relative pl-6 border-l-2 border-red-500/30 mb-8">
                <p className="text-xl font-dm-sans text-text-primary italic leading-relaxed">
                  "{item.original_bullet}"
                </p>
              </div>

              {/* Explanation */}
              <div className="flex items-start gap-4 mb-6 bg-brand-primary/5 p-5 rounded-xl border border-brand-primary/10">
                <AlertCircle size={20} className="text-[#8AA5FF] mt-0.5 shrink-0" />
        ...
        {bullets.length === 0 && (
          <div className="text-center p-12 text-text-secondary italic bg-text-primary/5 rounded-2xl border border-dashed border-border-primary">
