...
import { ChevronDown, CheckCircle2, AlertCircle, XCircle, Lightbulb } from 'lucide-react';
...
const SectionItem = ({ section }) => {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-all duration-300 mb-3 group ${isOpen ? 'bg-bg-card/80 border-brand-primary/30 shadow-[0_0_20px_rgba(41,52,255,0.1)]' : 'bg-bg-card/40 border-border-primary hover:border-border-strong hover:bg-bg-card/60'}`}>
      {/* Header (Clickable) */}
      <button
          ...
          >
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 bg-bg-dark/20">
              ...
              <div className="md:col-span-2 mt-2 pt-4 border-t border-border-primary">
                <p className="text-text-body text-sm bg-brand-primary/5 border border-brand-primary/10 p-4 rounded-xl flex items-start">
                  <Lightbulb size={16} className="text-brand-primary mr-2 mt-0.5 flex-shrink-0" aria-label="Impact" />
                  <span>{impact}</span>
                </p>
              </div>
