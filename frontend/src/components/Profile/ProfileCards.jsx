...
export const DataCard = ({ title, icon, children, className = "", ...props }) => ( // <-- 2. Accept ...props
  ...
  <motion.div
    className={cn(
      ...
      // Base Shape & Colors
      "rounded-2xl p-8 bg-bg-card",
      // Borders & Shadows (The Glow Effect)
      "border border-border-primary hover:scale-101 transition-transform duration-300 ease-in-out",
  ...
  >
    ...
    <div className="flex items-center gap-x-3 mb-6">
      ...
      <h3 className="text-2xl font-clash-display text-shadow-neutral-500 text-shadow-xs text-text-primary">{title}</h3>
    </div>
...
export const SkillTag = ({ children }) => (
  <span className="bg-bg-dark/50 text-text-primary border border-border-secondary text-sm font-bold px-3 py-1 rounded-full">
    {children}
  </span>
);
...
export const ExperienceItem = ({ item }) => (
  <div className="border-b border-border-primary pb-4 last:pb-0 last:border-b-0">
...
export const EducationItem = ({ item }) => (
  <div>
    <h4 className="font-semibold text-text-primary">{item.institution_name}</h4>
